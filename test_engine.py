...
class TestEngine:
    ...
    def ensure_structure(self):
        dirs = [
            ...
            "lang", "suites", "perf", 
            "sandbox/exp", "tests/unit", "tests/integration"
        ]
...
timestamp: {datetime.now().isoformat()}
        ...
        try:
            ...
            subprocess.run('gh issue list --json number | python3 -c "import json,sys;print(json.load(sys.stdin)[0][\'number\'])"', 
                         shell=True, capture_output=True, text=True, cwd=self.repo_path)
            subprocess.run(f'gh issue close 1 --comment "resolved"', 
    ...
    def multi_lang(self):
        ...
        try:
            self.exec_git([
                "git add lang/",
                f'git commit -m "multi-lang: {concept}"',
                "git push"
            ])
